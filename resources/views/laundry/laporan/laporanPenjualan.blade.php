  ...
  <div class="content-wrapper">
    ...
    <div class="box box-primary" style="margin-top:3%;">
      <div class="box-header with-border">
        <h3 class="box-title">Lihat Laporan Berdasarkan</h3>
      </div>
      <!-- /.box-header -->
      <!-- form start -->
      <div class="box-body">
        <div class="form-group">
              <?php
                  echo Form::open(['url' => 'laporan/penjualan/', 'method' => 'post']);
                    echo csrf_field() ;

                    echo "<div class='form-group'>";
                      echo ' <input type="checkbox" name="cbNota" ">'; echo' <label>No Nota</label>  <select class="form-control select2" name="noNota" style="width: 100%;">';
                      foreach ($dataPenjualan as $arr ){
                        echo '<option value="'. $arr->ID_NOTA.'">'. $arr->ID_NOTA.'</option>';
                      }echo '</select>';
                      ...
                      echo ' <input type="checkbox" name="cbPegawai" ">'; echo' <label>Pegawai</label>  <select class="form-control select2" name="idPegawai" style="width: 100%;">';
                      foreach ($dataPegawai as $arr ){
                        echo '<option value="'. $arr->ID_PEGAWAI.'">'. $arr->NAMA_PEGAWAI.'</option>';
                      }echo '</select>';

                      echo ' <input type="checkbox" name="cbCustomer"">'; echo' <label>Customer</label>  <select class="form-control select2" name="idCustomer" style="width: 100%;">';
